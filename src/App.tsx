    ...
    <>
      <main className="layout">
        <header className="topbar">
          <div className="topbar-meta">
            ...
            <p>Live chord progression performer and cover recorder</p>
          </div>
          <button
      ...
      <section className="panel performance-panel">
        ...
        <div className="controls">
          <button className="btn btn-primary" onClick={() => void playCurrentChord()}>
            Play (Space)
          </button>
          <button className="btn btn-secondary" onClick={prevChord}>
            Prev (←)
          </button>
          <button className="btn btn-secondary" onClick={nextChord}>
            Next (→)
          </button>
          <button
            className={`btn ${fermataEnabled ? 'btn-primary' : 'btn-secondary'}`}
            onClick={toggleFermata}
          >
            Fermata {fermataEnabled ? 'Latched' : fermataKeyHeld ? 'Held' : 'Off'} (F hold)
          </button>
          <button className={`btn ${isRecording ? 'btn-danger' : 'btn-secondary'}`} onClick={() => void toggleRecording()}>
        ...
        <p className="hint">
          Sampled instruments may take a second to load on first play. If loading fails, playback falls back to synth.
        </p>
        <p className="hint">
          Fermata mode extends current chord hold to about 2.5x your configured hold duration.
        </p>
